...
def compile_flags(configs: dict) -> str:
    ...
    if (
        configs["animate-opt"] is None
    ):  # set enable/disable animation flag to empty if it doesn't exist
        anim_flag = ""
    else:
        anim_flag = configs["animate-opt"][set_animation]
    if (
        flags is None and configs["animate-opt"] is None
    ):  # set flags to empty if there are no flags and enable/disable animation flags
        flags = ""
    elif (
        configs["animate-opt"][set_animation] is not None
    ):  # add flag for enabled/disabled animation if it exists
        anim_flag = configs["animate-opt"][set_animation]
...
def main():
    ...
    if not Path(dockerfile).exists():
        ...
        if Path(f"models/{MODEL}/requirements.txt").exists():
            sp.run(
                args=f"mv models/{MODEL}/requirements.txt {MODELS_DIR}/{dir_name}/",
                shell=True,
            )
    else:
