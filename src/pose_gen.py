...
from pandas import concat as dfconcat


def pose_gen(video, ots_file, model_type, sframe, fframe, joints, duration):
    ...
    OTS_pos, OTS_quat, OTS_theta, skpd_frames, torso_width, frame_stat = get_OTSData(
        ots_file, "GAIT", sframe, duration
    )
    ots_data = {"theta": OTS_theta, "pos": OTS_pos, "quat": OTS_quat}
    df_ots_raw = data_parse2("OTS", ots_data, joints)

    MODEL_pos, MODEL_theta, MODEL_quat = get_poseData(video, model_type, sframe, fframe)
    ...
    df_raw = dfconcat([df_ots_raw, df_m_raw], axis=1)
